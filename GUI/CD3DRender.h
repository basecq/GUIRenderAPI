...
{
public:
	CD3DFont( SIMPLEGUI_CHAR *szFontName, int fontHeight, DWORD dwCreateFlags = 0 );
	~CD3DFont();

	HRESULT Initialize(IDirect3DDevice9 *pD3Ddev);
	HRESULT Invalidate();

	HRESULT Print( float x, float y, DWORD colour, const SIMPLEGUI_CHAR *szText, DWORD dwFlags = 0 );

	int DrawLength( const SIMPLEGUI_CHAR* );
...
private:	
	...
	SIMPLEGUI_CHAR 	m_szFontName[31+1];
	int  	m_fontHeight;
