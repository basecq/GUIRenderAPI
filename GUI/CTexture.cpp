...
CTexture::CTexture( ID3DXSprite * pSprite, const SIMPLEGUI_CHAR * szPath )
{
	...
	tPath = new SIMPLEGUI_CHAR[SIMPLEGUI_STRLEN(szPath)+1];
	SIMPLEGUI_STRCPY_S(tPath, SIMPLEGUI_STRLEN(szPath)+1, szPath);
}

CTexture::CTexture( ID3DXSprite * pSprite, const SIMPLEGUI_CHAR * szPath, CColor *color )
{
	pSprite->GetDevice( &m_pDevice );
	D3DXCreateTextureFromFile( m_pDevice, szPath, &m_pTexture );
	if( !m_pTexture )
...
{
	//if( !m_pSprite || !m_vTexture ) return;
