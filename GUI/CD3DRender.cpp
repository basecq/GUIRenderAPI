...
CD3DFont::CD3DFont( SIMPLEGUI_CHAR *pszFontName, int iFontHeight, DWORD dwCreateFlags )
{
	SIMPLEGUI_STRCPY_S ( m_szFontName, 32, ( pszFontName ? pszFontName : _UI("Arial") ) );
...
HRESULT CD3DFont::Print( float x, float y, DWORD colour, const SIMPLEGUI_CHAR* szText, DWORD dwFlags )
{
	if( !m_isReady )
	{
	...
	if( *szText != _UI('\0') )
	{
		...
		int iLen = static_cast<int>( SIMPLEGUI_STRLEN ( szText ) );
...
int CD3DFont::DrawLength( const SIMPLEGUI_CHAR* szText )
{
	float fLen = 0.0f;

	for( int i = SIMPLEGUI_STRLEN ( szText ) - 1; i >= 0; i-- )
	{
