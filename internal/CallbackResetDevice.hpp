 ...
 *
 *      tAction manager for IDirect3DDevice9::Reset
 * 
 */
#pragma once
...
/*
 *  tAction Managers for notifying about D3D9 device reset
 */
...
struct ResetDevicePatcher //: public BasePatcher<Reset_t, 16>
{
	...
	injector::hook_back<Reset_t> hb[16];
    ...
    void Patch(int n, uintptr_t addr, HRESULT(*hf)(), size_t nopsize)
    {
        ...
        if(addr)
        {
            ...
            hb[n].fun = injector::MakeCALL(addr, raw_ptr(hf)).get();
